...
public:
        ...
        : Node("contest1_node")
    {
        ...
        startYaw = 0.0;
        error = 0.0;
        kp = 0.15; // Proportional gain for wall following - should test the math here 
...
private:
    ...
    void wallFollowingRoutine()
    {
        ...
        bool any_bumper_pressed = checkBumpers();
        float error = minRightLaserDist_ - rightWallMax; 
        float kp = 0.15; // Proportional gain for wall following - should test the math here
        ...
        if (minLaserDist_ < frontTooClose) // front obstacle too close
        {
        ...
        else if (minLaserDist_ > frontTooClose && minRightLaserDist_ > rightWallMax) // clear in front but no wall on right side
        {
            angular_ = -kp * (minRightLaserDist_ - rightWallMax); // turn right to find wall
            linear_ = 0.2;
            velocityPublish();
                                //go back to wall searching
         ...
         else  // clear in front and wall on right side
        {
            ...
            if (error <= 0.0) // too close to right wall, with 20cm buffer
            {
                linear_ = 0.2;
                angular_ = - kp * error; // turn left slightly
                velocityPublish();
            }
            else if (error >= 0.0) // too far from right wall
            {
                linear_ = 0.2;
                angular_ = - kp * error; // turn right slightly
                velocityPublish();
            }
