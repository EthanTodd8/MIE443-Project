...
public:
        ...
        : Node("contest1_node")
    {
        ...
        maxLaserDist_ = 0.0; //initialize maximum laser distance to 0 (ensures all other measurements will be larger)
        maxLaserDist_idx_ = 0; //initialize index of maximum laser distance measurement
        ...
        minimumObstacleDistance = 05; //minimum distance the robot can have to an obstacle in any direction before it is considered too close and needs to turn away
        ...
        // frontRight_distance_ = std::numeric_limits<float>::infinity();
        // frontLeft_distance_ = std::numeric_limits<float>::infinity();
...
private:
    ...
    void laserCallback(const sensor_msgs::msg::LaserScan::SharedPtr scan)
    {
        ...
        */
        ...
        right_idx_ = 0; //find index of right obstacle distance measurement
        frontRight_idx_ = nLasers_ / 8;
        frontLeft_idx_ = nLasers_ * 3 / 8;
        ...
        maxLaserDist_ = 0.0; //find the furthest obstacle to the current position of the robot FROM front_idx_ - desiredNLasers_ to front_idx_ + desiredNLasers_ (NOT FULL SCAN AROUND ROBOT)
        if (deg2rad(desiredAngle_) < scan->angle_max &&deg2rad(desiredAngle_) > scan->angle_min) { 
            for (uint32_t laser_idx = front_idx_ - desiredNLasers_; laser_idx < front_idx_ + desiredNLasers_; ++laser_idx) {
                if (laserRange_[laser_idx] < 0.20) { continue; } // ignore laser measurements that are less than 20cm, which are less than turtlebot radius
                else if (laserRange_[laser_idx] > maxLaserDist_) { maxLaserDist_ = laserRange_[laser_idx]; maxLaserDist_idx_ = laser_idx; } // find maximum laser distance within desired angle range
                else if (laserRange_[laser_idx] < minLaserDist_) {minLaserDist_ = laserRange_[laser_idx]; minLaserDist_idx_ = laser_idx;} // find minimum laser distance within desired angle range
            }
        } else {
        ...
        frontRight_distance_ = laserRange_[frontRight_idx_];
        frontLeft_distance_ = laserRange_[frontLeft_idx_];

        createLasersArray(); // Define filteredLaserRange array with values from laserRange_ above 20cm

        RCLCPP_INFO(this->get_logger(), "front_distance_: %f, right_distance_: %f, left_distance: %f, back_distance: %f, frontRight_distance_: %f, frontLeft_distance_: %f" , front_distance_, right_distance_, left_distance_, back_distance_, frontRight_distance_, frontLeft_distance_);
    }
    ...
    void startupRoutine()
    {
        ...
        linear_ = 0.2;
        angular_ = 0.0;
        if (front_distance_ < 0.6) {callstartupRoutine = false;}
    }
    ...
    void randomRoutine()
    {
        ...
        if (front_distance_ > 0.6 && !randomTurn) 
        {
            linear_ = 0.2;
            angular_ = 0.0;
            return;
    ...
    void turnRobot(bool _turnClockwise)
    {
        ...
        if (distanceDifference > 0.01 && abs(normalizeAngle(start_yaw_ - yaw_)) < deg2rad(135))  // rotate until minimum laser distance is on beside robot, or to a max turn angle of 90 degrees
        { 
            linear_ = 0.0;
    ...
    void moveBack()
    {
        ...
        if (distance_traveled >= 0.30 || back_distance_ < 0.40)
        {
    ...
    bool tooCloseToObstacle()
    {
        ...
        bool useObstacleHandling = false;
        if (frontRight_distance_ < minimumObstacleDistance)
        {
            angular_ = 0.1;
            linear_ = 0.1;
            useObstacleHandling = true;
        }
        else if (frontLeft_distance_ < minimumObstacleDistance)
        {
            angular_ = -0.1;
            linear_ = 0.1;
            useObstacleHandling = true;
    ...
    void controlLoop()
    {
        ...
        {       
            ...
            max_element = maxLaserDist_;
            max_element_idx = maxLaserDist_idx_;

            // Determine turn direction based on index of maximum distance
            turnClockwise = max_element_idx < (front_idx_); // true = turn right, false = turn left
            startup = false;
    ...
    float frontRight_distance_;
    float frontLeft_distance_;
    int front_idx_;
    int back_idx_;
    int right_idx_;
    int left_idx_;
